...
export default function Home() {
  ...
  useEffect(() => {
    window.__walletAddress = walletAddress ?? null
  }, [walletAddress])

  // Restore previously connected wallet on page load (no popup)
  useEffect(() => {
  ...
  const connectWallet = useCallback(async () => {
    if (!window.ethereum) { setNoWallet(true); return }
    ...
    try {
      ...
      if ((e as { code?: number }).code !== 4001) console.error('Wallet error:', e)
    } finally {
      ...
      <div className="w-full max-w-[800px] mt-2 rounded-lg px-4 py-2.5 border"
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1">
          {[
            ...
            { label: 'High Vol >$5M', desc: 'Speed boost!', color: '#FF8844' },
          ].map(t => (
